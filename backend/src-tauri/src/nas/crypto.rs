...
pub fn session_encryption_key_from_env() -> Result<Vec<u8>, String> {
    let raw = std::env::var("TELEGRAM_SESSION_ENCRYPTION_KEY")
        .map_err(|_| "TELEGRAM_SESSION_ENCRYPTION_KEY is required".to_string())?;
    let value = raw.trim();
    if value.is_empty() {
        return Err("TELEGRAM_SESSION_ENCRYPTION_KEY is required".to_string());
    }
    ...
    let decoded = STANDARD_NO_PAD
        ...
        .map_err(|_| {
            "TELEGRAM_SESSION_ENCRYPTION_KEY must be a 32-byte base64 or hex encoded key"
                .to_string()
        })?;

    if decoded.len() != 32 {
        return Err("TELEGRAM_SESSION_ENCRYPTION_KEY must decode to exactly 32 bytes".to_string());
    }

    Ok(decoded)
}

pub fn issue_jwt(claims: &AuthClaims, signing_key: &[u8]) -> Result<String, String> {
    encode(
        &Header::default(),
        claims,
