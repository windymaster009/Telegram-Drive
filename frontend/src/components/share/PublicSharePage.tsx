...
import { publicShareApi, type PublicShareView } from '../../lib/publicShareApi';
...
export function PublicSharePage({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    publicShareApi.get(token)
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : 'Could not open this share link');
      })
      .finally(() => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-300">
                  <ShieldCheck className="h-3.5 w-3.5" /> Read-only share
                </div>
                <h1 className="mt-1 truncate text-xl font-semibold text-white sm:text-2xl">{share.label}</h1>
              </div>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs text-slate-400">
          ...
          <>
            ...
            {files.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                {files.map((file) => (
                  <PublicFileCard key={file.id} file={file} token={token} onOpen={() => setSelected(file)} />
                ))}
              </div>
            )}
          </>
          ...
          <div className="flex h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0b1420] shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-slate-500">{formatBytes(selected.size || 0)}</div>
              </div>
              <div className="flex items-center gap-2">
                <a
                  href={publicShareApi.mediaUrl(token, selected.id, true)}
                  className="flex h-10 items-center gap-2 rounded-xl bg-cyan-400 px-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"
                >
                  <Download className="h-4 w-4" />
                  <span className="hidden sm:inline">Download</span>
                </a>
                <button onClick={() => setSelected(null)} className="grid h-10 w-10 place-items-center rounded-xl bg-white/5 text-slate-300 transition hover:bg-white/10 hover:text-white">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto p-2 sm:p-4">
              <PublicPreview file={selected} token={token} />
            </div>
          </div>
        </div>
      )}
...
function PublicShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen overflow-auto bg-[radial-gradient(circle_at_top_left,_#142238,_#08101d_52%,_#050810)] text-white">
      {children}
      <footer className="mx-auto max-w-7xl px-4 pb-8 pt-4 text-center text-xs text-slate-600 sm:px-6">
        Shared securely with Telegram Drive
      </footer>
    </div>
  );
}

function FileDetails({ file, token, onPreview }: { file: TelegramFile; token: string; onPreview: () => void }) {
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
          ...
          {file.created_at && <span>{file.created_at}</span>}
          {file.mime_type && <span>{file.mime_type}</span>}
        </div>
      </div>
      <div className="flex gap-2">
        <button onClick={onPreview} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-sm font-medium text-slate-200 transition hover:bg-white/5 sm:flex-none">
          <Play className="h-4 w-4" /> Preview
        </button>
        <a href={publicShareApi.mediaUrl(token, file.id, true)} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300 sm:flex-none">
...
function PublicFileCard({ file, token, onOpen }: { file: TelegramFile; token: string; onOpen: () => void }) {
  const mediaUrl = publicShareApi.mediaUrl(token, file.id);
  return (
    <article className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035] transition hover:-translate-y-0.5 hover:border-cyan-400/30 hover:bg-white/[0.055]">
      <button onClick={onOpen} className="block w-full text-left">
        <div className="relative aspect-[4/3] overflow-hidden bg-[#101b28]">
          {isImage(file) ? (
            <img src={mediaUrl} alt="" loading="lazy" className="h-full w-full object-cover" />
          ) : (
            <div className="flex h-full w-full items-center justify-center">
              <FileTypeIcon file={file} />
              {isVideo(file) && (
                <span className="absolute grid h-10 w-10 place-items-center rounded-full bg-black/65 text-white shadow-lg">
        ...
        <div className="px-3 pb-2 pt-3">
          <h3 className="line-clamp-2 min-h-10 text-sm font-semibold leading-5 text-white">{file.name}</h3>
          <div className="mt-1 text-xs text-slate-500">{formatBytes(file.size || 0)}</div>
        </div>
      </button>
      <div className="px-3 pb-3">
        <a
          href={publicShareApi.mediaUrl(token, file.id, true)}
          onClick={(event) => event.stopPropagation()}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 py-2 text-xs font-semibold text-slate-300 transition hover:border-cyan-400/30 hover:bg-cyan-400/10 hover:text-cyan-200"
        >
...
function PublicPreview({ file, token }: { file: TelegramFile; token: string }) {
  const url = publicShareApi.mediaUrl(token, file.id);
  const ext = extension(file);

  if (isImage(file)) {
    return <img src={url} alt={file.name} className="max-h-[78vh] max-w-full rounded-xl object-contain shadow-2xl" />;
  }
  if (isVideo(file)) {
    return <video src={url} controls playsInline preload="metadata" className="max-h-[78vh] max-w-full rounded-xl bg-black shadow-2xl" />;
  }
  if (isAudio(file)) {
    return (
      <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-white/[0.04] p-6 text-center">
        <FileTypeIcon file={file} />
        <div className="mt-4 text-sm font-semibold">{file.name}</div>
        <audio src={url} controls preload="metadata" className="mt-5 w-full" />
      </div>
    );
  }
  if (ext === 'pdf') {
    return <iframe src={url} title={file.name} className="h-[78vh] w-full rounded-xl border-0 bg-white shadow-2xl" />;
  }
  if (ext === 'docx') {
    return <WordDocumentViewer file={file} activeFolderId={null} streamUrlOverride={url} />;
  }
  if (EXCEL_EXTENSIONS.has(ext)) {
    return <ExcelWorkbookViewer file={file} activeFolderId={null} streamUrlOverride={url} />;
  }
  if (TEXT_EXTENSIONS.has(ext)) {
    return <PublicTextPreview url={url} />;
  }
  ...
  return (
    <div className="flex max-w-lg flex-col items-center rounded-3xl border border-white/10 bg-white/[0.04] px-8 py-12 text-center">
      ...
      <a href={publicShareApi.mediaUrl(token, file.id, true)} className="mt-5 flex items-center gap-2 rounded-xl bg-cyan-400 px-5 py-2.5 text-sm font-semibold text-slate-950">
        <Download className="h-4 w-4" /> Download
      </a>
    </div>
  );
...
function PublicTextPreview({ url }: { url: string }) {
  ...
  return (
    <pre className="max-h-[78vh] w-full max-w-5xl overflow-auto whitespace-pre-wrap break-words rounded-xl bg-white p-5 font-mono text-xs leading-6 text-slate-800 shadow-2xl sm:p-8 sm:text-sm">
